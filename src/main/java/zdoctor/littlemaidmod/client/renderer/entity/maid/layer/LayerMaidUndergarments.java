...
public class LayerMaidUndergarments implements LayerRenderer<EntityLivingBase> {
	private RendererServant<? extends EntityMaid> maidRender;

	public LayerMaidUndergarments(RendererServant<? extends EntityMaid> maidRenderIn) {
	...
	public void doRenderLayer(EntityLivingBase entitylivingbaseIn, float limbSwing, float limbSwingAmount,
		...
		if (!entitylivingbaseIn.isInvisible() && !((IMaid) entitylivingbaseIn).hasUniform()) {
			this.maidRender.bindTexture(Config.Variables.UG_VARIANTS[((EntityMaid) entitylivingbaseIn).getUndergarments()]);
